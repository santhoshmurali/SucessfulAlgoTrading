...
"""
# Load Excel file
excel_file = os.getenv('TradingSystem','DhanTrading_v3.xlsx')
...
LIMIT_PRICE = "I"
INPUT_PROP = "J"
QUANTITY = "K" #Order Quantity 
ORD_ACTIVATED = "L" #If Order is activated for CE
LTP_ = "H"
SCRIPT_KEY = "G"
BUY_ORDER_ID = "N"
BUY_ORDER_STATUS = "O"
...
def place_buy_order(dhan,script_key, order_qty, trade_price,correlation_id):
    ...
    SLICE_ORDER_BUY = dhan.place_order(
...
def get_order_details(dhan):
    # Buy Order Object
    BUY_ORDER = [None for k in range(11)]

    # Sell Profit Object
    PROFIT_ORDER = [None for k in range(11)]

    # Sell Loss Object
    LOSS_ORDER = [None for k in range(11)]
    ...
    while True:
        if TradeSheet.range("Generate_PL").value:
            ...
            last_row = OrdersSheet.range("A1").end('down').row
            OrdersSheet.range(f"A1:S{last_row}").clear_contents()
            # ✅ CRITICAL: Verify get_trade_book() response structure in 2.1.0
            trade_book_response = dhan.get_trade_book()
            if trade_book_response['status'] == 'success':
                OrdersSheet.range("A1").options(index=False).value = pd.DataFrame(trade_book_response['data'])
            else:
                print(f"⚠️ Trade book fetch failed: {trade_book_response['remarks']}")

        if TradeSheet.range("Safe_Mode").value:
            time.sleep(0.5)


        for strikes in range(11): # Change this to 11 for all 10 strikes + ATM
            ...
            if (TradeSheet.range(f'{QUANTITY}{strikes+6}').value and TradeSheet.range(f'{ORD_ACTIVATED}{strikes+6}').value == False and TRADE_COMPLETE == False): 
                ...
                #ltp__ = (TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value if (TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value and TradeSheet.range('LIMITMODE').value)  else TradeSheet.range(f'{LTP_}{strikes+6}').value)
                if (TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value > 0 and TradeSheet.range('LIMITMODE').value==True):
                    ltp__ = TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value
                    if TradeSheet.range(f'{LTP_}{strikes+6}').value > ltp__:
                        ...
                        TradeSheet.range(f'{ORDER_QTY}{strikes+6}').value = None
                #elif (TradeSheet.range('LIMITMODE').value==False and TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value == None) :
                elif (TradeSheet.range(f'{LIMIT_PRICE}{strikes+6}').value == None and TradeSheet.range('LIMITMODE').value==False): 
            ...
            if (TradeSheet.range(f'{QUANTITY}{strikes+6}').value and TradeSheet.range(f'{ORD_ACTIVATED}{strikes+6}').value == True and 
                ...
                Quantity = TradeSheet.range(f'{ORDER_QTY}{strikes+6}').value
                ...
                if ltp_>=trigger_profit_ and not(PROFIT_ORDER[strikes]) and not(SWING_SWITCH):
                    ...
                    PROFIT_ORDER[strikes] = place_profit_order(dhan,Script_Key,TRADED_QUANTITY,profit_price_,"") # I'll have to revisit and check If this can be replaced with Quantity
                    SWING_SWITCH = True
                ...
                if ltp_<=trigger_sl_ and not(LOSS_ORDER[strikes]) and not(SWING_SWITCH):
                    ...
                    LOSS_ORDER[strikes] = place_sl_order(dhan,Script_Key,TRADED_QUANTITY,sl_price_,"")    # I'll have to revisit and check If this can be replaced with Quantity
                    SWING_SWITCH = True                                              
            ...
            # Stage 5 : cancel the order that is not traded when allocation % in excel is removed and reset all flags
            # if (not(TradeSheet.range(f'{QUANTITY}{strikes+6}').value) and TradeSheet.range(f'{ORD_ACTIVATED}{strikes+6}').value == True and TRADE_COMPLETE == False and TradeSheet.range(f'{BUY_ORDER_STATUS}{strikes+6}').value != "TRADED" and TradeSheet.range(f'{BUY_ORDER_ID}{strikes+6}').value != None): 
            #     if (BUY_ORDER[strikes] and BUY_ORDER[strikes]['status']=='success'):
            #         CANCEL_ORD = place_cancel_order(dhan,str(BUY_ORDER[strikes]['data']['orderId']))
            #         ORDER_CANCELLED = False
            #         while CANCEL_ORD and not(ORDER_CANCELLED):
            #             cancel_order_id = CANCEL_ORD['data']['orderId']
            #             time.sleep(0.5)
            ...
            #             if cancel_order_status == 'CANCELLED':
            #                 ORDER_CANCELLED = True
            #                 BUY_ORDER[strikes] = None
            #     rest_flags_values(strikes, "TRADE CANCELLED")
            #     TRADED_QUANTITY = None
            #     TRADE_COMPLETE = False
            #     SWING_SWITCH = False
                # ce_col_activated = chr(ord(CE_COL)+strikes)
                # if (
...
def main():
    ...
    if len(sys.argv) < 2:
        print("❌ Usage: python order_mgmt lite vx..py [dev|prod]")
        sys.exit(1)
