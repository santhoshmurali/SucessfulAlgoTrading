...
def initial_sheet_config(sandbox):
    ...
    dhan_context = connections__['context']  # ✅ ADDED: Get context
    ...
    return({
        ...
        "client_id":client_id,
        "access_token" : access_token,
        "context": dhan_context  # ✅ ADDED: Return context
    })
...
def run_feed(dhan_context,dhan):
    """
    ...
    try:
        ...
        data = MarketFeed(dhan_context, prepared_instruments, version) 
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("Usage: python PValuePulseData_v[x].py [dev|prod]")
        sys.exit(1)
        
    print(f"Starting in {sys.argv[1]} mode...")
    ...
    dhan_context = set_up_worksheet['context']  # ✅ CHANGED: Get context
    while True:
        run_feed(dhan_context,dhan) # ✅ CHANGED: Pass context instead of cid, atoken
