...
def load_lotsize_from_env():
    try:
        ...
        freezeqty_str = os.getenv('freeze_qty','{}')
        # Parse each string separately
        lotsize_dict = ast.literal_eval(lotsize_str)
        freezeqty_dict = ast.literal_eval(freezeqty_str)
        return {
            'lotsize': lotsize_dict,
            'freezeqty': freezeqty_dict,
        }
...
def initial_sheet_config():
    ...
    filtered_df_index['LOT_SIZE'] = filtered_df_index['SEM_TRADING_SYMBOL'].map(lotsize['lotsize'])
    filtered_df_index['FREEZE_QTY'] = filtered_df_index['SEM_TRADING_SYMBOL'].map(lotsize['freezeqty'])
    filtered_df_index['FREEZE_LOT'] = (filtered_df_index['FREEZE_QTY'] / filtered_df_index['LOT_SIZE']).astype(int)

    IndexLookup.clear()
    IndexLookup.range('A1').options(index=False).value = filtered_df_index[['SEM_TRADING_SYMBOL','SEM_SMST_SECURITY_ID','LOT_SIZE','FREEZE_QTY','FREEZE_LOT']]
    #--------------------------------------------------------------------------------------------
...
def refresh_instruments(change_instrument, dhan):
    ...
    #itm_4_key = str(int(TradeSheet.range("ITM_FOUR_KEY").value))
    #itm_5_key = str(int(TradeSheet.range("ITM_FIVE_KEY").value))
    #itm_6_key = str(int(TradeSheet.range("ITM_SIX_KEY").value))
    #itm_7_key = str(int(TradeSheet.range("ITM_SEVEN_KEY").value))
    #itm_8_key = str(int(TradeSheet.range("ITM_EIGHT_KEY").value))
    #itm_9_key = str(int(TradeSheet.range("ITM_NINE_KEY").value))
    #itm_10_key = str(int(TradeSheet.range("ITM_TEN_KEY").value))

    #----------------------
    # Get the LTP of selected Instrument in ORDERMGMT SHEET
    #-------------------------
    selected_instrument_key = str(int(OrderMgmt.range("SYMBOL_KEY").value))


    #--------------------
    # Range mapping for selected instruments
    #--------------------
    ...
    range_mapping = {
        ...
        #itm_4_key : 'ITM_FOUR_LTP',
        #itm_5_key : 'ITM_FIVE_LTP', 
        #itm_6_key : 'ITM_SIX_LTP',
        #itm_7_key : 'ITM_SEVEN_LTP',    
        #itm_8_key : 'ITM_EIGHT_LTP',
        #itm_9_key : 'ITM_NINE_LTP',
        #itm_10_key : 'ITM_TEN_LTP',
        selected_instrument_key : 'LTP',
    }
...
def run_feed(clientid,accesstoken,dhan):
    ...
    try:
        ...
        SERIES_CHECK = OrderMgmt.range('SYMBOL_KEY').value
        ...
        while True:
            ...
            if ( STRIKE_CHECK != TradeSheet.range('ITM_ONE_KEY').value or SERIES_CHECK != OrderMgmt.range('SYMBOL_KEY').value):
                refresh_start_time = time.time()
                instruments_all = refresh_instruments(False,dhan)  # Update instruments by calling subscription_management
                instruments = list(instruments_all.keys())
                prepared_instruments = prepare_instruments(instruments)
                ...
                STRIKE_CHECK = TradeSheet.range('ITM_ONE_KEY').value
                SERIES_CHECK = OrderMgmt.range('SYMBOL_KEY').value
                print("Subscription updated based on Strike Change or Series Change.")           
            ...
            if response['type'] == 'Ticker Data' and response['security_id'] != int(instruments[0]) :
                ...
                OrderMgmt.range(cell_ref).value = response['LTP']
